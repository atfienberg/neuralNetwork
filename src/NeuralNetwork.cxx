...
NeuralNetwork::NeuralNetwork(const char* filename){
  ...
  else if(netMap.at("activation function").string_value() == "rectified linear"){
    setRectifiedLinear();
  }
  else{
  ...
  for(index i = 0; i < sizes.size() - 1; ++i){				
    auto thisInterface = netMap.at("interface " + std::to_string(i)).object_items();    
    ...
    for(index j = 0; j < thisJsonWeightMatrix.size(); ++j){
      if(weightMatrices_[i][j].size() != thisJsonWeightMatrix[j].array_items().size()){
	inconsistentJsonError();
      }
...
void NeuralNetwork::saveToFile(const char* filename) const{
  ...
  outputMap["learning parameters"] = json11::Json::object { 
    {"eta", eta_},
    {"mu", mu_},
    {"lambda", lambda_} };
  ...
  for(index i = 0; i < weightMatrices_.size(); ++i){
    ...
    outputMap[interfaceLabel.c_str()] = json11::Json::object { 
      {"weight matrix", weightMatrices_[i]},
      {"biases", biasVectors_[i]} };      
  }
...
void NeuralNetwork::initializeWeightsAndBiases(const std::vector<int>& sizes, bool randomize){
  ...
  for(index matrixNum = 1; matrixNum < sizes.size(); ++matrixNum){
	...
	double startWeightValue =  randomizeLayer ? 
	  rand->Gaus(0,1) / std::sqrt(sizes[matrixNum-1]) : 0;
	thisMatrix[rowNum][colNum] = startWeightValue;
      } //for colNum      
      double startBiasValue = randomizeLayer ? 
	rand->Gaus(0,1) : 0;
